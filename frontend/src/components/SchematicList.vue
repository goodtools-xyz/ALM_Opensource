
<template>
  <div class="schematic-list">
    <div class="toolbar">
      <el-select v-model="selectedProject" placeholder="选择项目">
        <el-option label="全部" value="" />
        <el-option v-for="p in projects" :key="p.projectId" :label="p.name" :value="p.projectId" />
      </el-select>
      <el-button type="primary" @click="showAddModal = true">
        <el-icon><component :is="icons.Plus" /></el-icon>
        上传原理图
      </el-button>
    </div>
    
    <el-table :data="schematics" border>
      <el-table-column prop="schematicId" label="原理图ID" width="120" />
      <el-table-column prop="projectId" label="项目ID" width="120" />
      <el-table-column prop="name" label="文件名称" />
      <el-table-column prop="filePath" label="文件路径" />
      <el-table-column prop="fileType" label="文件类型" width="100" />
      <el-table-column prop="version" label="版本" width="80" />
      <el-table-column prop="status" label="状态" width="100">
        <template #default="scope">
          <el-tag :type="getStatusTagType(scope.row.status)">{{ scope.row.status }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="createdBy" label="创建人" width="100" />
      <el-table-column prop="createdAt" label="创建时间" width="180" />
      <el-table-column label="操作" width="200">
        <template #default="scope">
          <el-button size="small" @click="previewSchematic(scope.row)">预览</el-button>
          <el-button size="small" @click="editSchematic(scope.row)">编辑</el-button>
          <el-button size="small" type="danger" @click="deleteSchematic(scope.row.schematicId)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 添加/编辑弹窗 -->
    <el-dialog v-model="showAddModal" :title="editSchematicData ? '编辑原理图' : '上传原理图'">
      <el-form :model="formData" label-width="100px">
        <el-form-item label="项目">
          <el-select v-model="formData.projectId">
            <el-option v-for="p in projects" :key="p.projectId" :label="p.name" :value="p.projectId" />
          </el-select>
        </el-form-item>
        <el-form-item label="文件名称">
          <el-input v-model="formData.name" />
        </el-form-item>
        <el-form-item label="文件路径">
          <el-input v-model="formData.filePath" />
        </el-form-item>
        <el-form-item label="文件类型">
          <el-input v-model="formData.fileType" />
        </el-form-item>
        <el-form-item label="版本">
          <el-input v-model="formData.version" />
        </el-form-item>
        <el-form-item label="状态">
          <el-select v-model="formData.status">
            <el-option label="DRAFT" value="DRAFT" />
            <el-option label="REVIEW" value="REVIEW" />
            <el-option label="APPROVED" value="APPROVED" />
          </el-select>
        </el-form-item>
        <el-form-item label="创建人">
          <el-input v-model="formData.createdBy" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showAddModal = false">取消</el-button>
        <el-button type="primary" @click="saveSchematic">保存</el-button>
      </template>
    </el-dialog>

    <!-- 预览弹窗 -->
    <el-dialog v-model="showPreviewModal" title="原理图预览" width="800px">
      <div class="preview-content">
        <p>文件名称: {{ previewData?.name }}</p>
        <p>文件路径: {{ previewData?.filePath }}</p>
        <p>版本: {{ previewData?.version }}</p>
        <div class="preview-placeholder">
          <el-icon size="100"><component :is="icons.FileText" /></el-icon>
          <p>原理图预览区域</p>
        </div>
      </div>
    </el-dialog>
  </div>
</template>

<script setup>
import { ref, reactive, markRaw, onMounted, watch } from 'vue'
import { Plus, Files } from '@element-plus/icons-vue'
import { hardwareAPI } from '../api'

const icons = { Plus: markRaw(Plus), FileText: markRaw(Files) }

const projects = ref([])
const schematics = ref([])
const selectedProject = ref('')
const showAddModal = ref(false)
const showPreviewModal = ref(false)
const editSchematicData = ref(null)
const previewData = ref(null)

const formData = reactive({
  projectId: '',
  name: '',
  filePath: '',
  fileType: 'SCH',
  version: '1.0',
  status: 'DRAFT',
  createdBy: ''
})

const loadProjects = async () => {
  try {
    const response = await hardwareAPI.getProjects()
    projects.value = response.data
  } catch (error) {
    console.error('加载项目失败:', error)
  }
}

const loadSchematics = async () => {
  try {
    if (selectedProject.value) {
      const response = await hardwareAPI.getSchematics(selectedProject.value)
      schematics.value = response.data
    } else {
      schematics.value = []
    }
  } catch (error) {
    console.error('加载原理图失败:', error)
  }
}

const getStatusTagType = (status) => {
  switch (status) {
    case 'DRAFT': return 'info'
    case 'REVIEW': return 'warning'
    case 'APPROVED': return 'success'
    default: return 'default'
  }
}

const previewSchematic = (schematic) => {
  previewData.value = schematic
  showPreviewModal.value = true
}

const editSchematic = (schematic) => {
  editSchematicData.value = schematic
  formData.projectId = schematic.projectId
  formData.name = schematic.name
  formData.filePath = schematic.filePath
  formData.fileType = schematic.fileType || 'SCH'
  formData.version = schematic.version || '1.0'
  formData.status = schematic.status
  formData.createdBy = schematic.createdBy || ''
  showAddModal.value = true
}

const deleteSchematic = async (id) => {
  try {
    await hardwareAPI.deleteSchematic(id)
    loadSchematics()
  } catch (error) {
    console.error('删除原理图失败:', error)
  }
}

const saveSchematic = async () => {
  try {
    if (editSchematicData.value) {
      await hardwareAPI.updateSchematic(editSchematicData.value.schematicId, formData)
    } else {
      await hardwareAPI.createSchematic(formData)
    }
    showAddModal.value = false
    editSchematicData.value = null
    formData.projectId = ''
    formData.name = ''
    formData.filePath = ''
    formData.fileType = 'SCH'
    formData.version = '1.0'
    formData.status = 'DRAFT'
    formData.createdBy = ''
    loadSchematics()
  } catch (error) {
    console.error('保存原理图失败:', error)
  }
}

watch(selectedProject, () => {
  loadSchematics()
})

onMounted(() => {
  loadProjects()
})
</script>

<style scoped>
.schematic-list {
  padding: 10px;
}

.toolbar {
  margin-bottom: 20px;
  display: flex;
  gap: 10px;
}

.preview-content {
  padding: 20px;
}

.preview-placeholder {
  margin-top: 20px;
  padding: 40px;
  text-align: center;
  background-color: #f5f5f5;
  border-radius: 8px;
}
</style>
